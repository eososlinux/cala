...
def run_grub_install(fw_type, partitions, efi_directory, install_hybrid_grub):
    ...
    if fw_type == "efi":
        ...
        # Determine the disk device to install GRUB (BIOS mode)
        boot_loader_install_path = None
        
        # If install_hybrid_grub is True, we might need to find the ESP disk
        # even if bootLoader is set (which is typical in BIOS-only installs)
        if install_hybrid_grub:
            efi_system_partition = libcalamares.globalstorage.value("efiSystemPartition")
            if efi_system_partition:
                # Find the partition that matches the ESP mount point
                esp_partition = next((p for p in partitions if p.get("mountPoint") == efi_system_partition), None)
                if esp_partition:
                    # Use the existing get_partition_drive function to find the disk
                    # get_partition_drive expects a device path like /dev/sda1
                    device = esp_partition.get("device")
                    if device:
                        drive, _ = get_partition_drive(device)
                        boot_loader_install_path = drive
        
        # Fallback to bootLoader info if not found via ESP
        if not boot_loader_install_path:
            if libcalamares.globalstorage.value("bootLoader") is None:
                libcalamares.utils.warning(_("Cannot find the drive to install GRUB to!"))
                return
...
def install_grub(efi_directory, fw_type, install_hybrid_grub):
    ...
    # Determine if we should install GRUB EFI
    install_efi = fw_type == "efi" or install_hybrid_grub
    
    # For hybrid installations on BIOS systems, efi_directory must be available
    if install_hybrid_grub and fw_type == "bios" and not efi_directory:
        libcalamares.utils.warning(_("Hybrid GRUB installation requested, but no EFI system partition is available. "
                                   "Please configure an EFI partition for hybrid boot support."))
        install_efi = False  # Skip EFI installation
...
def run():
    ...
    # Ensure install_hybrid_grub is a boolean True, not just a truthy value
    install_hybrid_grub = install_hybrid_grub is True
